...
vi.mock('../../../src/renderer/src/features/workspace/components/TerminalNode', () => {
  return {
    TerminalNode: ({ onStop, onRerun }: { onStop?: () => void; onRerun?: () => void }) => {
      return (
        <div>
          <button type="button" data-testid="terminal-rerun" onClick={() => onRerun?.()}>
            Rerun
          </button>
          <button type="button" data-testid="terminal-stop" onClick={() => onStop?.()}>
            Stop
          </button>
        </div>
      )
...
describe('WorkspaceCanvas agent launch guard', () => {
  it('kills launched PTY when stop happens during an in-flight rerun', async () => {
    ...
    fireEvent.click(screen.getByTestId('terminal-rerun'))
    ...
    expect(agentNode?.data.status).toBe('stopped')
  })
})
