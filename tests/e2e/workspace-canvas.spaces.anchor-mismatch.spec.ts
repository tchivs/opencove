        ...
        .poll(async () => {
          return await window.evaluate(
            async ({ key, nodeId, spaceId }) => {
              ...
              const workspace = parsed.workspaces?.[0]
              const taskNode = workspace?.nodes?.find(node => node.id === nodeId)
              const space = workspace?.spaces?.find(item => item.id === spaceId)

              return {
                moved:
                  typeof taskNode?.position?.x === 'number' &&
                  typeof taskNode?.position?.y === 'number' &&
                  (taskNode.position.x !== 220 || taskNode.position.y !== 220),
